        ...
        <form action="{{ route('admin.reference.kecamatan.update', $kecamatan->id) }}" method="POST">
                ...
                <div class="col-md-6 mb-3">
                    ...
                    <select id="provinsi" class="form-select" name="provinsi_id" required>
                        ...
                        @foreach ($provinces as $province)
                            <option value="{{ $province->id }}" {{ old('provinsi_id', $kecamatan->kabupaten->provinsi_id) == $province->id ? 'selected' : '' }}>
            ...
            <div class="row">
                <div class="col-md-6 mb-3">
                    ...
                    @enderror
                    <small class="text-muted">Kode Kecamatan</small>
                </div>
            ...
            <div class="row">
                <div class="col-md-6 mb-3">
                    ...
                    <select class="form-select @error('status') is-invalid @enderror" id="status" name="status" required>
                        ...
                        <option value="1" {{ (old('status', $kecamatan->status) == '1' || old('status', $kecamatan->status) === true || old('status', $kecamatan->status) === 1) ? 'selected' : '' }}>Aktif</option>
                        <option value="0" {{ (old('status', $kecamatan->status) == '0' || old('status', $kecamatan->status) === false || old('status', $kecamatan->status) === 0) ? 'selected' : '' }}>Nonaktif</option>
                    </select>
                    @error('status')
                        <div class="invalid-feedback">{{ $message }}</div>
                    @enderror
...
<script>
    ...
    async function loadKabupaten(provinceId, selectValue = null) {
        ...
        try {
            ...
            if (cache.kabupaten[provinceId]) {
                console.log('Using cached kabupaten for', provinceId);
                populateKabupaten(cache.kabupaten[provinceId], selectValue);
                return;
            }
            ...
            kabupatenEl.innerHTML = '<option value="">-- Error loading data: ' + error.message + ' --</option>';
        }
    }
    ...
    function populateKabupaten(data, selectValue = null) {
        ...
        for (const item of data) {
            ...
            // Set selected jika ada selectValue atau merupakan kabupaten saat ini
            const valueToSelect = selectValue || "{{ old('kabupaten_id', $kecamatan->kabupaten_id) }}";
    ...
    function resetForm() {
        ...
        document.getElementById('nama_kecamatan').value = initialValues.namaKecamatan;
        
        // Reload kabupaten dropdown dan set ke initial value
        loadKabupaten(initialValues.provinsiId, initialValues.kabupatenId);
    }
